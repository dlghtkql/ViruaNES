...
inline	WORD	CPU::RD6502W( WORD addr ) //RD6502W
{
	...
	if( PROM_ACCESS ) {
		INT	ofs = CPU_MEM_PAGE[addr>>13]*0x2000;	// 8K
		*((WORD*)&PROM_ACCESS[ofs+(addr&0x1FFF)]) = 0xFFFF;
	}
#endif
...
#if	0
#ifdef	_DEBUG
	::memmove( &_PCTRACE[1], _PCTRACE, sizeof(WORD)*(256-1) );
	_PCTRACE[0] = R.PC;
#endif
{
	//추가 설명 필요
...
DEBUGOUT( "BIN DUMP\n" );훃
		if( (fp = ::fopen( "mainram.bin", "wb" )) != NULL ) {
			...
			fwrite( CPU_MEM_BANK[5], 8*1024, 1, fp );
			fwrite( CPU_MEM_BANK[6], 8*1024, 1, fp );
			fclose(fp);
		} else {
DEBUGOUT( "BIN DUMP ERR...\n" );
		}
	}
}
